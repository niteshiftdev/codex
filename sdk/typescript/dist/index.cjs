...
async function createOutputSchemaFile(schema) {
  ...
  try {
    await import_node_fs.promises.writeFile(schemaPath, JSON.stringify(schema), "utf8");
...
var CodexExec = class {
  ...
  async *run(args) {
    ...
    const child = (0, import_node_child_process.spawn)(this.executablePath, commandArgs, {
      env
    });
    let spawnError = null;
...
var scriptFileName = typeof import_meta !== "undefined" && import_meta.url ? (0, import_node_url.fileURLToPath)(import_meta.url) : __filename;
var scriptDirName = import_node_path2.default.dirname(scriptFileName);
function findCodexPath() {
  ...
  const vendorRoot = import_node_path2.default.join(scriptDirName, "..", "vendor");
  const archRoot = import_node_path2.default.join(vendorRoot, targetTriple);
  const codexBinaryName = process.platform === "win32" ? "codex.exe" : "codex";
  const binaryPath = import_node_path2.default.join(archRoot, "codex", codexBinaryName);
  return binaryPath;
}
